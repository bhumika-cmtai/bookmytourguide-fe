...
import { Search, Plus, Edit, Loader2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function GuidesAdminPage() {
  ...
  const { guides, pagination, loading, error } = useSelector((state: RootState) => state.guide);
        ...
        <tr key={guide._id}>
          <td className="px-6 py-4">
            <div className="flex items-center gap-4">
              <img src={guide.photo || '/placeholder.png'} alt={guide.name} className="w-12 h-12 rounded-full object-cover" />
              <div>
          ...
          <td className="px-6 py-4 text-center">
            <Badge variant={guide.isApproved ? 'default' : 'destructive'} className="cursor-pointer" onClick={() => handleToggleApproval(guide._id, guide.isApproved)}>
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage, approve, and view all registered tour guides.</p>
          </div>
        ...
        <div className="bg-card rounded-lg border shadow-sm overflow-hidden">
          <table className="w-full">
            <thead className="bg-muted/50">
              <tr>
                <th className="px-6 py-3 text-left uppercase tracking-wider text-sm">Guide</th>
                <th className="px-6 py-3 text-center uppercase tracking-wider text-sm">Profile %</th>
                <th className="px-6 py-3 text-center uppercase tracking-wider text-sm">Status</th>
                <th className="px-6 py-3 text-right uppercase tracking-wider text-sm">Actions</th>
              </tr>
            </thead>
      ...
      {editingGuide && (
        <EditGuideSheet
          ...
          onToggleApproval={() => handleToggleApproval(editingGuide._id, editingGuide.isApproved)}
        />
      )}
    </div>
  );
